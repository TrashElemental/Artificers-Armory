...
public class ExamineMobTask implements FamiliarTask {
    ...
    public boolean canRun(FamiliarEntity familiar) {
        ...
        List<Mob> valid = mobs.stream().filter(m -> !(m instanceof Monster)).filter(LivingEntity::isAlive).toList();
    ...
    public void tick(FamiliarEntity familiar) {
        ...
        Vec3 targetPos = target.position().add(xOffset, yOffset, zOffset);
